    ...
    if (showRetrievingPopup) {
      const timer = setTimeout(() => {
        fetch("http://localhost:3001/userData", {
          ...
          .then((res) => res.json())
          .then((data) => {
            // Check if API returned valid user data with a userType
            if (data.status === "ok" && data.data && data.data.userType) {
              const userType = data.data.userType.toLowerCase().trim();
              console.log(userType, "usertype");
              let redirectUrl = "";
              if (userType === "doctor" || userType === "physician") {
                redirectUrl = "/admin/default";
              } else if (userType === "nurse") {
                redirectUrl = "/admin/default/nurse";
              } else if (userType === "patient") {
                redirectUrl = "/admin/default/patient";
              } else if (userType === "admin") {
                redirectUrl = "/admin/default/admin";
              } else if (userType === "super admin") {
                redirectUrl = "/admin/default/superadmin";
              } else if (userType === "lab technician") {
                redirectUrl = "/admin/default/labtechnician";
              } else if (userType === "pharmacist") {
                redirectUrl = "/admin/default/pharmacist";
              }
              if (redirectUrl) {
